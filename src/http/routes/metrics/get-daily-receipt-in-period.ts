...
export const getDailyReceiptInPeriod = new Elysia().use(auth).get(
  ...
  async ({ getCurrentUser, query, set }) => {
    ...
    const startDate = from ? dayjs(from) : dayjs().subtract(7, 'days') // inicio do intervalo de 7 dias
    const endDate = to ? dayjs(to) : from ? startDate.add(7, 'days') : dayjs() // fim do intervalo de 7 dias

    // Se a diferença entre as datas for maior que 7 dias, retorna erro
